...
public class UserGoalService {
    ...
    public void deleteUserGoalById(int id) {
        Optional<UserGoal> userGoalOptional = userGoalRepository.findByGoalId(id);
        if (userGoalOptional.isPresent()) {
            userGoalRepository.deleteById(id);
        } else {
            throw new RuntimeException("UserGoal not found");
        }
    }
